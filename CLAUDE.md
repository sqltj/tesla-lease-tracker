...
- **Component location:** If component was added to a wrong location (e.g. stored into `src/components` instead of `src/tesla-lease-tracker/ui/components`), move it to the proper folder
- **Component organization:** Prefer grouping components by functionality rather than by file type (e.g. `src/tesla-lease-tracker/ui/components/chat/`)

## Project Structure
Full-stack app: `src/tesla-lease-tracker/ui/` (React + Vite) and `src/tesla-lease-tracker/backend/` (FastAPI). Backend serves frontend at `/` and API at `/api`. API client auto-generated from OpenAPI schema.
...
- **Components:** Use shadcn/ui, add to `src/tesla-lease-tracker/ui/components/`
